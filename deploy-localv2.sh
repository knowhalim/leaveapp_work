...
#####################################################################
# Simple HR Leave System - Deploy from Local to VPS (v2)
#
# Uploads files directly from your local machine to a fresh VPS
# and sets everything up. No GitHub required.
#
# v2 changes vs deploy-local.sh:
#   - Works with non-root sudo users (e.g. the 'aisg' Azure VM user)
#   - All remote sections run via "sudo bash -s" instead of "bash -s"
#   - INSTALL_DIR is created with sudo then chowned to SSH_USER so
#     rsync can upload without needing root on the rsync connection
#   - Default SSH user changed from root to aisg
#
# Usage:
...
#   - The SSH user must have passwordless sudo, OR sudo must not
#     require a TTY (set "Defaults !requiretty" in /etc/sudoers).
#     Azure Ubuntu VMs grant passwordless sudo to the default user
#     by default, so this should work out of the box.
#####################################################################
...
# Create INSTALL_DIR as root, then recursively chown to SSH_USER so rsync can write.
# On redeployments the directory will be owned by www-data from the previous run,
# so we always chown -R to restore write access for the aisg user before rsync.
$SSH_CMD "sudo mkdir -p ${INSTALL_DIR} && sudo chown -R ${SSH_USER}:${SSH_USER} ${INSTALL_DIR}"
...
# NOTE: "sudo bash -s" runs the entire heredoc as root.
# This works because Azure Ubuntu VMs grant passwordless sudo to the default user.
$SSH_CMD "sudo bash -s" << 'REMOTE_DEPS'
set -e
...
# Use array cache temporarily so artisan can boot before migrations create the cache table.
# A settings service provider reads from the cache table on every boot, which fails if
# the table doesn't exist yet. We restore this to database after migrations run.
...
echo "==> Installing PHP deps (--no-scripts skips package:discover so it doesn't run before migrations)..."
...
echo "==> Clearing stale bootstrap cache (prevents PailServiceProvider not found errors)..."
...
echo "==> Running migrations (must happen before package:discover which reads cache table)..."
sudo -u www-data php artisan migrate --force
sudo -u www-data php artisan db:seed --force

echo "==> Restoring database cache driver now that migrations have created the cache table..."
sed -i "s|CACHE_STORE=.*|CACHE_STORE=database|g" .env

echo "==> Running package:discover now that the cache table exists..."
...
$SSH_CMD "sudo bash -s" << REMOTE_FINAL
set -e
